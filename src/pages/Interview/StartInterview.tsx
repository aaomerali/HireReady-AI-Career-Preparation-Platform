...
const InterviewPage = () => {
  ...
  // FIX #1: Changed state.interviews to state.interview to match your RootState
  ...
  // منطق التحقق من اكتمال جميع الأسئلة
  ...
  // Helper to determine if current question is saved
  const isCurrentQuestionSaved = savedQuestionsIndices.includes(activeIndexState);

  // 1. Fetch Data (Interview + User Answers) using Thunks
  useEffect(() => {
  ...
  const setActiveIndex = (newIndex: number) => {
    if (newIndex !== activeIndexState) {
      ...
      if (!canMove) {
        window.alert("Please answer the current question and save it before moving to another question.");
        return;
      }
  ...
  // 2. Generate AI Feedback using Thunk
  const generateFeedbackAndSave = useCallback(async (answer: string) => {
    if (!answer.trim() || answer.trim().length < 20) {
      window.alert("Please provide an answer with more than a few words (e.g., at least 20 characters) before generating feedback.");
      return;
    }
  ...
  const saveAnswer = async () => {
    ...
    if (!interview || !finalAnswer.trim() || !aiResult) {
      window.alert("Please record/type and generate feedback before saving.");
      return;
    }

    if (!userId) {
      const proceed = window.confirm("You are not authenticated. Save logic requires User ID.");
      if (!proceed) return;
    }
    ...
    try {
      ...
      if (err.message) {
        ...
        window.alert("Error saving answer.");
      }
    } finally {
      setSaving(false);
    }
    ...
    <div className="p-6 space-y-8 max-w-5xl mx-auto">
      ...
      <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
        ...
        <p className="text-gray-600 mt-2">
          ...
          The interview consists of {interview.questions?.length || 0} questions. You’ll receive a personalized report based
          on your responses at the end.
          <strong className="block mt-2 text-red-700">NOTE: You must Save the answer before moving to the next question. Saved questions cannot be modified.</strong>
        </p>
      </div>
      ...
      <div>
        <div className="flex gap-3 mb-4 overflow-x-auto pb-2">
          {/* FIX #2: Added explicit type annotation for (item, index) to prevent 'implicit any' error */}
        ...
        <div className="mt-2 p-4 border rounded-md bg-white shadow">
          ...
          <div className="flex justify-end mt-3">
            ...
            >
              <Play size={16} /> Read Question Aloud
            </button>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex gap-3 mt-4 flex-wrap justify-center">
                <button
                  onClick={() => { setIsWebcamOn((p) => !p); if (!isWebcamOn) setVideoEnabled(true); }}
                ...
                <>
                  <div className="flex gap-3 mb-4">
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg ${inputMode === "voice"
                    ...
                    <button
                      ...
                      disabled={isRecording}
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg ${inputMode === "keyboard"
                        ? "bg-blue-600 text-white hover:bg-blue-500"
                        : "bg-gray-200 text-gray-800 hover:bg-gray-300"}`}
                    >
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {inputMode === "voice" ? (
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg cursor-pointer ${isRecording
                      ...
                      <button
                        onClick={toggleRecording}
                        disabled={isAiGenerating || !typedAnswer.trim()}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg ${isAiGenerating || !typedAnswer.trim()
                          ? "bg-indigo-300 text-white cursor-not-allowed"
                          : "bg-indigo-600 text-white hover:bg-indigo-500 cursor-pointer"}`}
                      >
                        {isAiGenerating ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
                        Submit Answer
                      </button>
                    )}
                    ...
                    <button
                      ...
                      disabled={!aiResult}
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg ${aiResult
                ...
                {inputMode === "voice" && !isCurrentQuestionSaved ? (
                  <>
                    {userAnswer || interimResult ? (
                      <p className="text-gray-700 whitespace-pre-wrap">{userAnswer || interimResult}</p>
                    ) : (
                      <p className="text-gray-400 italic">Start recording to see your answer here...</p>
                    )}
                    {interimResult && (
                  ...
                  <textarea
                    ...
                    disabled={isAiGenerating || isCurrentQuestionSaved}
                    placeholder="Type your answer here..."
                    rows={5}
                    className="w-full p-2 border rounded-md focus:ring-indigo-500 focus:border-indigo-500 bg-white"
                  />
                ) : (
                  <p className="text-gray-700 whitespace-pre-wrap">{userAnswer || typedAnswer || "No answer recorded/typed."}</p>
                )}



                {isCurrentQuestionSaved && !aiResult && (
                  <div className="mt-4 p-3 bg-gray-100 rounded-md border text-center">
                    <p className="text-gray-500 text-sm">Feedback for this saved question is hidden or archived.</p>
                  </div>
                )}
      ...
      {/* 2. زر الإنهاء في الأسفل */}
      <div className="flex justify-end mt-8 pt-6 border-t border-gray-200">
        <button
          onClick={handleFinishInterview}
          disabled={!allQuestionsAnswered}
          className={`flex items-center gap-3 px-8 py-4 rounded-lg font-bold text-lg transition-all shadow-sm w-full md:w-auto justify-center
          ${allQuestionsAnswered
              ? "bg-purple-600 text-white hover:bg-purple-700 hover:shadow-lg cursor-pointer transform hover:-translate-y-1"
              : "bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200"
            }`}
        >
          <CheckCircle size={24} />
          {allQuestionsAnswered ? "Finish Interview & See Report" : "Answer All Questions to Finish"}
        </button>
      </div>
