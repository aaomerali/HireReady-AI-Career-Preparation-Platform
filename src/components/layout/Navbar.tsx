...
function Navbar() {
  ...
  // مصفوفة تحتوي على القوائم وأسمائها
  const navItems = [
    { name: "Dashboard", path: "/dashboard" },
    { name: "Interviews", path: "/interview/all" },
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t shadow-md flex flex-col p-4 gap-4 animate-fadeIn">
          ...
          <div className="flex gap-3 pt-2">
            <button className="flex-1 py-2 rounded-md bg-[hsl(210,40%,96.1%)] font-medium" onClick={() => alert("Edit Account")}>
              Edit Account
            </button>
            <button className="flex-1 py-2 rounded-md bg-red-600 text-white font-semibold shadow-md" onClick={handleLogout}>
              Log Out
            </button>
          </div>
        </div>
